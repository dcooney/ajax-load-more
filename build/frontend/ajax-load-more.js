...
function getCacheSlug(alm, data) {
	...
	if (addons.nextpage) {
		...
		// Standard.
		return md5_default()(JSON.stringify(data)).toString();
	}
}
...
let imagesLoaded = __webpack_require__(564);
...
/* harmony default export */ var helpers_almDomParser = (almDomParser);
...
;// CONCATENATED MODULE: ./src/frontend/js/modules/fadeIn.js
/**
 * Fade element in
 * @param {*} element
 * @param {*} speed
...
/* harmony default export */ var fadeIn = (almFadeIn);

;// CONCATENATED MODULE: ./src/frontend/js/modules/fadeOut.js
/**
 * Fade element out
 * @param {*} element
 * @param {*} speed
...
/* harmony default export */ var fadeOut = (almFadeOut);
...
let masonry_imagesLoaded = __webpack_require__(564);
	...
	return new Promise((resolve) => {
		...
		if (!filtering) {
			...
			if (masonry_init && init) {
				...
				masonry_imagesLoaded(container, function () {
					...
					setTimeout(function () {
						...
						// Fade In
						fadeIn(container.parentNode, 125);
			...
			// Standard / Append content.
				...
				let data = helpers_stripEmptyNodes(helpers_almDomParser(html, 'text/html'));
...
// External Modules
let qs = __webpack_require__(129);
let ajax_load_more_imagesLoaded = __webpack_require__(564);
...
(function () {
	...
	const ajaxloadmore = function (el, index) {
		...
		if (alm.addons.filters === 'true') {
			...
			let page = helpers_getParameterByName('pg');
				...
				if (!alm.addons.paging) {
					...
					if (alm.addons.single_post) {
						...
						if (!alm.transition_container) {
							...
							alm.el = alm.html;
							reveal = alm.container_type === 'table' ? helpers_tableWrap(alm.html) : helpers_stripEmptyNodes(helpers_almDomParser(alm.html, 'text/html'));
						} else {
							// Standard container
							let pagenum;
							...
							if (alm.init && (alm.start_page > 1 || alm.addons.filters_startpage > 0)) {
								...
								const html = helpers_stripEmptyNodes(helpers_almDomParser(alm.html, 'text/html'));
					...
					if (!alm.init) {
						if (pagingContent) {
							fadeOut(pagingContent, alm.speed);
							...
							setTimeout(function () {
								...
								ajax_load_more_imagesLoaded(pagingContent, function () {
									...
									fadeIn(pagingContent, alm.speed);
